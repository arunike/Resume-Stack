...
export function Education({ data, isLatex }: EducationProps) {
    const { school, degree, location, date, relevantCourses, schoolFontSize = 16, degreeFontSize = 14, locationFontSize = 12, dateFontSize = 12, relevantCoursesFontSize = 12,
        schoolBold = true, degreeBold = true, locationBold = true, dateBold = true, relevantCoursesLabelBold = true, relevantCoursesBold = false
    } = data
        ...
        <div className={`w-full ${isLatex ? 'font-serif' : ''}`}>
            ...
            {relevantCourses && (
                <div className="mt-1" style={{ marginTop: 'var(--relevant-courses-spacing, 4px)' }}>
                    <span
                        className={clsx('text-zinc-900 mr-2', relevantCoursesLabelBold && 'font-bold')}
